...
namespace Repository
{
    public class AuthenticationRepository : IAuthenticationRepository
    {
        ...
        public async Task<ResponseViewModellogin> addAppUser(AddAppUserViewModel addAppUser)
        {
            ...
            //parameters.Add("@LoginID", "###", DbType.String);
            ...
            addAppUser.Mobile = addAppUser.Mobile?
                .Trim()
                .Replace(" ", "")
                .Replace("+91", "");
            ...
            if (string.IsNullOrWhiteSpace(addAppUser.Password) ||
                addAppUser.Password.Length < 8)
            {
                return new ResponseViewModellogin
                {
                    statusCode = (int)HttpStatusCode.BadRequest,
            ...
            // Name Validation
            var nameRegex =
                new System.Text.RegularExpressions.Regex(@"^(?![0-9]+$)[A-Za-z0-9\s]+$");
            ...
            {
                var result =
                    await connection.QueryFirstOrDefaultAsync<EmailActionModel>(
            ...
            using (var connection = _dapperContext.createConnection())
            {
                try
                {
                    ...
                    if (insertedUser != null && intResult > 0)
                    {
                        string authLogin =
                            insertedUser.AuthLogin != null
                            ? insertedUser.AuthLogin.ToString()
                            : "";
                        ...
                        if (welcomeResult != null &&
                            welcomeResult.statusCode == 1)
                        {
                            plainPassword =
                                welcomeResult.AuthPass != null
                                ? welcomeResult.AuthPass.ToString()
                                : "";

                            string name =
                                addAppUser.FName + " " + addAppUser.LName;

                            _emailService
                                .SendOtpEmailForUserRegistrationWelcomletter(
                                    authLogin,
                                    plainPassword,
                                    addAppUser.Email,
                                    name,
