...
namespace Repository
{
    public class SelfRepository : ISelfRepository
    {
        ...
        public async Task<Result2<AddFundModel>> SItoBalanceAsync(RequestWalletAddressModel model)
        {
            ...
            try
            {
                ...
                using (var conn = _dapperContext.createConnection())
                {
                    ...
                    string? SitoBalance = await encryptDecrypt.GetSITOBalance(modelObj.WalletAddress, privateKey);
            
                    modelObj.SitoBalance = SitoBalance;

                    // Step 4: Check USDT Balance and BNB top-up
                    if (Convert.ToDouble(modelObj.SitoBalance) > 4)
                    {
